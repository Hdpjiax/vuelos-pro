  ...
  return (
    <>
      ...
      < aside className="fixed inset-y-0 left-0 z-40 hidden h-dvh w-72 shrink-0 flex-col border-r border-white/10 bg-sky-950 px-5 py-6 text-white md:flex" >
        ...
        <nav className="min-h-0 flex-1 space-y-2 overflow-y-auto overscroll-contain pr-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Navegación principal">
          {items.map((item) => {
            const isActive = item.href === activeHref;
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-cyan-500 text-white shadow-lg shadow-cyan-950/25 ring-1 ring-cyan-200 hover:bg-cyan-400 hover:text-white [&>svg]:text-white"
