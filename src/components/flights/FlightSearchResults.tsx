...
import { Plane, Clock, Users, AlertCircle, Zap, Luggage } from 'lucide-react';
...
type Props = {
  data: { data: FlightOffer[]; dictionaries?: { carriers?: Record<string, string> } } | null;
  error?: string;
  searchSummary?: { originLabel: string; destinationLabel: string; adults: number; departureDate: string; returnDate?: string };
};
...
const CABIN_LABEL: Record<string, string> = {
  ECONOMY: 'Económica', PREMIUM_ECONOMY: 'Premium Eco', BUSINESS: 'Business', FIRST: 'Primera clase',
};
      ...
      {searchSummary && (
        <div className="flex flex-wrap items-center gap-2 rounded-2xl bg-sky-50 px-4 py-3">
          ...
          <p className="text-sm font-black text-sky-800">
            {searchSummary.originLabel} → {searchSummary.destinationLabel}
          </p>
          <span className="text-sky-400">·</span>
          <p className="text-sm text-sky-700">{searchSummary.adults} adulto{searchSummary.adults > 1 ? 's' : ''}</p>
          <span className="text-sky-400">·</span>
            ...
            <div className="flex flex-col gap-5 md:flex-row md:items-stretch">
              ...
              <div className="flex-1 space-y-4">
                {[{ itin: outbound, label: inbound ? 'Ida' : '' }, ...(inbound ? [{ itin: inbound, label: 'Regreso' }] : [])].map(({ itin, label }) => {
                  ...
                  return (
                    <div key={label + seg0.departure.at} className="flex items-center gap-3">
                      {/* Aerolínea */}
                      <div className="flex h-10 w-10 shrink-0 flex-col items-center justify-center rounded-2xl bg-sky-50 text-center">
                        <span className="text-[11px] font-black text-sky-700 leading-none">{seg0.carrierCode}</span>
                        {label && <span className="mt-0.5 text-[9px] font-bold text-slate-400 uppercase">{label}</span>}
                      </div>

                      {/* Ruta */}
                      <div className="flex flex-1 items-center gap-2">
                        <div className="text-center">
                          <p className="text-xl font-black text-slate-900 leading-none">{formatTime(seg0.departure.at)}</p>
                        ...
                        <div className="flex flex-1 flex-col items-center gap-0.5 px-2">
                          <div className="flex items-center gap-1 text-[10px] font-bold text-slate-400">
                            <Clock size={10} />{parseDuration(itin.duration)}
                          </div>
                          <div className="flex w-full items-center gap-1">
                            <div className="h-px flex-1 bg-slate-200" />
                            <Plane size={12} className="text-sky-400 rotate-90" />
                            <div className="h-px flex-1 bg-slate-200" />
                          </div>
                          {itinStops === 0
                        ...
                        <div className="text-center">
                          <p className="text-xl font-black text-slate-900 leading-none">{formatTime(segLast.arrival.at)}</p>
                ...
                <div className="flex flex-wrap gap-2">
                  <span className={`rounded-xl px-2.5 py-1 text-[11px] font-black ${CABIN_COLOR[cabin] ?? 'bg-slate-100 text-slate-600'}`}>
                    {CABIN_LABEL[cabin] ?? cabin}
                  </span>
                  <span className="flex items-center gap-1 rounded-xl bg-slate-100 px-2.5 py-1 text-[11px] font-bold text-slate-600">
                    <Luggage size={11} />{bags > 0 ? `${bags} maleta incluida` : 'Sin maleta'}
                  </span>
                  <span className="flex items-center gap-1 rounded-xl bg-slate-100 px-2.5 py-1 text-[11px] font-bold text-slate-600">
                    <Users size={11} />{offer.numberOfBookableSeats} asientos
                  </span>
                  {offer.numberOfBookableSeats <= 4 && (
                    <span className="flex items-center gap-1 rounded-xl bg-rose-100 px-2.5 py-1 text-[11px] font-black text-rose-600">
                      <Zap size={10} />¡Últimos!
                    </span>
                  )}
                </div>
              ...
              {/* Precio */}
              <div className="flex shrink-0 flex-col items-center justify-center rounded-2xl border border-sky-100 bg-gradient-to-b from-sky-50 to-white px-6 py-5 md:min-w-[150px]">
                ...
                <p className="text-xs text-slate-400">{offer.travelerPricings.length} pasajero{offer.travelerPricings.length > 1 ? 's' : ''}</p>
