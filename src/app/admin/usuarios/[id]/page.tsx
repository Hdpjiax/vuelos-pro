...
import { buttonPrimary, buttonSecondary, buttonSecondarySmall, inputClass, labelClass } from "@/lib/styles";
...
export default async function AdminUserDetailPage({ params, searchParams }: PageProps) {
  ...
  const [{ data: profile }, { data: flights }, { data: logs }] = await Promise.all([
    supabase.from("profiles").select("id, full_name, email, role, created_at").eq("id", id).single(),
    supabase
      .from("flights")
      .select("id, flight_type, flight_date, flight_time, return_flight_date, return_flight_time, fare_type, total_amount, payment_percentage, amount_to_pay, final_amount_to_pay, status, created_at")
      .eq("user_id", id)
  ...
  const { data: messages } = flightIds.length
    ? await supabase
  ...
  const totalConfirmed = safeFlights.reduce((sum: number, flight: any) => {
    return ["pago_confirmado", "pendiente_qr", "qr_enviado", "completado"].includes(flight.status)
      ? sum + getAmountToPay(flight)
      : sum;
  }, 0);
...
function StatMini({ icon, title, value, helper }: { icon: React.ReactNode; title: string; value: React.ReactNode; helper: string }) {
  return (
