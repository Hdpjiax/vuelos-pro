...
import { type FlightExtras, type FlightType, type Passenger } from "@/lib/types";
import { updateUserFlightAction, type EditFlightFormState } from "./actions";

const initialState: EditFlightFormState = {};

type PassengerDraft = Passenger & {
  id: string;
};
...
type EditFlightFormProps = {
  flight: {
    ...
    extras: FlightExtras | null;
  };
};

function makePassengers(passengers: Passenger[] | null | undefined): PassengerDraft[] {
  const list = Array.isArray(passengers) && passengers.length
    ? passengers
    : [{ full_name: "", document: "", phone: "", birth_date: "", nationality: "" }];
  return list.map((passenger) => ({ ...passenger, id: crypto.randomUUID() }));
}

export function EditFlightForm({ flight }: EditFlightFormProps) {
  const [state, formAction, pending] = useActionState(updateUserFlightAction, initialState);
  const [flightType, setFlightType] = useState<string>(flight.flight_type === "redondo" ? "redondo" : "sencillo");
  const [passengers, setPassengers] = useState<PassengerDraft[]>(() => makePassengers(flight.passengers));
  const extras = flight.extras ?? {};
  const [checkedBags, setCheckedBags] = useState(String(extras.checked_bags ?? 0));
  const [carryOnBags, setCarryOnBags] = useState(String(extras.carry_on_bags ?? 0));
  const [seats, setSeats] = useState(String(extras.seats ?? 0));
  ...
  function addPassenger() {
    setPassengers((current) => [
      ...current,
      { id: crypto.randomUUID(), full_name: "", document: "", phone: "", birth_date: "", nationality: "" },
    ]);
  }
    ...
    <form action={formAction} className="space-y-6">
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-xl shadow-slate-200/60 backdrop-blur">
        <div className="mb-6 flex flex-col gap-2">
          <p className="text-sm font-black uppercase tracking-[0.24em] text-sky-700">Editar vuelo</p>
          <h3 className="text-2xl font-black tracking-tight text-slate-950">Información principal</h3>
          <p className="text-sm text-slate-500">
            Puedes editar un vuelo mientras siga pendiente de revisión o esperando pago. Si ya se había enviado cuenta bancaria, volverá a revisión.
          </p>
        </div>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="space-y-2">
            ...
            <input className={inputClass} type="date" name="flight_date" defaultValue={flight.flight_date} required />
          </label>

          <label className="space-y-2">
            <span className={labelClass}>Horario de ida</span>
            <input className={inputClass} type="time" name="flight_time" defaultValue={String(flight.flight_time).slice(0, 5)} required />
          </label>

          <label className="space-y-2">
            <span className={labelClass}>Tipo de tarifa</span>
            <select className={inputClass} name="fare_type" required defaultValue={flight.fare_type}>
          ...
          {flightType === "redondo" ? (
            <>
              <label className="space-y-2">
                ...
                <input className={inputClass} type="date" name="return_flight_date" defaultValue={flight.return_flight_date ?? ""} required />
              </label>

              <label className="space-y-2">
                <span className={labelClass}>Horario de regreso</span>
                <input className={inputClass} type="time" name="return_flight_time" defaultValue={String(flight.return_flight_time ?? "").slice(0, 5)} required />
              </label>
            </>
          ) : (
            <>
          ...
          <label className="space-y-2">
            ...
            <input className={inputClass} type="number" min="1" step="0.01" name="total_amount" defaultValue={Number(flight.total_amount ?? 0)} required />
          </label>
        </div>
      </section>
          ...
          {passengers.map((passenger, index) => (
            <div key={passenger.id} className="rounded-3xl border border-slate-200 bg-slate-50/70 p-4">
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <button type="button" onClick={() => removePassenger(passenger.id)} disabled={passengers.length === 1} className={`${buttonSecondarySmall} hover:border-rose-200 hover:text-rose-600`}>
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                <label className="space-y-2 xl:col-span-2">
                  ...
                  <input className={inputClass} name="passenger_full_name" defaultValue={passenger.full_name ?? ""} required={index === 0} />
                </label>

                <label className="space-y-2">
                  <span className={labelClass}>Documento / ID</span>
                  <input className={inputClass} name="passenger_document" defaultValue={passenger.document ?? ""} />
                </label>

                <label className="space-y-2">
                  <span className={labelClass}>Nacimiento</span>
                  <input className={inputClass} type="date" name="passenger_birth_date" defaultValue={passenger.birth_date ?? ""} />
                </label>

                <label className="space-y-2">
                  <span className={labelClass}>Nacionalidad</span>
                  <input className={inputClass} name="passenger_nationality" defaultValue={passenger.nationality ?? ""} />
                </label>

                <label className="space-y-2 md:col-span-2 xl:col-span-1">
                  <span className={labelClass}>Teléfono</span>
                  <input className={inputClass} name="passenger_phone" defaultValue={passenger.phone ?? ""} />
                </label>
              </div>
            </div>
          ))}
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-xl shadow-slate-200/60 backdrop-blur">
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          <label className="space-y-2">
            ...
            <input className={inputClass} name="other_extras" defaultValue={extras.other ?? ""} />
          </label>

          <label className="space-y-2">
            <span className={labelClass}>Notas para administración</span>
            <textarea className={`${inputClass} min-h-28 resize-y`} name="notes" defaultValue={extras.notes ?? ""} />
          </label>
        </div>
      </section>
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-xl shadow-slate-200/60 backdrop-blur">
        <div className="mb-5">
          <p className="text-sm font-black uppercase tracking-[0.24em] text-sky-700">Captura opcional</p>
          <h3 className="mt-1 text-2xl font-black tracking-tight text-slate-950">Reemplazar foto o captura</h3>
          <p className="mt-1 text-sm text-slate-500">Déjalo vacío si quieres conservar la imagen actual.</p>
        </div>
        ...
        <label className="flex cursor-pointer flex-col items-center justify-center gap-3 rounded-3xl border border-dashed border-sky-300 bg-sky-50/70 px-6 py-10 text-center transition hover:bg-sky-50">
          ...
          <span className="text-sm font-black text-slate-900">Seleccionar nueva imagen</span>
          <span className="text-xs font-semibold text-slate-500">PNG, JPG, WEBP o similar · máximo 5 MB</span>
          <input className="sr-only" type="file" name="flight_image" accept="image/*" />
        </label>
      </section>
